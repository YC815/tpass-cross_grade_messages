...
import { MessageSquareText, Settings, UserCog, Users, Webhook } from "lucide-react";
    ...
    <div className="min-h-full flex flex-col">
      ...
      <div className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 py-8 flex gap-8">
        <nav className="hidden md:flex w-44 shrink-0 flex-col gap-2">
          <AdminNavLink href="/admin" icon={<MessageSquareText className="h-4 w-4" />}>
            訊息紀錄
          </AdminNavLink>
          <AdminNavLink href="/admin/users" icon={<UserCog className="h-4 w-4" />}>
            使用者
          </AdminNavLink>
          <AdminNavLink href="/admin/webhooks" icon={<Webhook className="h-4 w-4" />}>
            Webhook
          </AdminNavLink>
          <AdminNavLink href="/admin/settings" icon={<Settings className="h-4 w-4" />}>
            設定
          </AdminNavLink>
          {superAdmin && (
